...
void KEYPAD_vidGetPressedKey(u8 *Add_pu8KeyValue)
{
	...
	for (Local_u8ColsIterator = 0; Local_u8ColsIterator < KEYPAD_COLS_NUM;
			Local_u8ColsIterator++)
	{

		DIO_enuSetPinValue(KEYPAD_COL_PORT,
				KEYPAD_u8ArrColsPins[Local_u8ColsIterator], DIO_u8_LOW);
		for (Local_u8RowsIterator = 0; Local_u8RowsIterator < KEYPAD_ROWS_NUM;
				Local_u8RowsIterator++)
		{

			DIO_enuGetPinValue(KEYPAD_ROW_PORT,
					KEYPAD_u8ArrRowsPins[Local_u8RowsIterator],
					&Local_u8PinValue);
			...
			if (Local_u8PinValue == 0)
			{
				...
				*Add_pu8KeyValue =
						KEYPAD_u8KeysValue[Local_u8RowsIterator][Local_u8ColsIterator];
		...
		DIO_enuSetPinValue(KEYPAD_COL_PORT,
				KEYPAD_u8ArrColsPins[Local_u8ColsIterator], DIO_u8_HIGH);
	}
